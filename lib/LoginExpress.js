...
const getUserModel = require('./models/getUserModel');
...
class LoginExpress {
  ...
  constructor({
    ...
    this.userModel = getUserModel(mongoDbModelName, mongoDbSchemaDefinition);
  }
  ...
  isAdmin = async (req, res, next) => {
    ...
    if (req.user.auth !== 'ADMIN') {
      res
        .status(403)
        .send('User does not have permission to access this resource.');
    }
  ...
  getUser = async (id) => {
    return await this.userModel
      .findById(id)
      .select('-password -verifyEmailToken -resetToken');
  };
