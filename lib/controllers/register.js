...
exports.signUpUser = (
	...
	try {
		...
		const sendMailLink = `<h4><a href="https://${req.headers.host}/verify-email/${token}">Verify Email Address</a></h4>`;
		...
		`;
		const sendMailSubjectLine = 'Please Verify Your Email Address';

		return user.updateOne(
			{ verifyEmail: false, verifyEmailToken: token },
			(err, success) => {
				if (err) {
					return res
						.status(400)
