...
exports.sendResetLink = (
	...
	try {
		...
		const sendMail = async (token) => {
			const link = `<h4><a href="https://${req.headers.host}/reset/${token}">Reset Password</a></h4>`;
			const output = `
        <div style="margin: auto; width: 40%; padding: 10px">
          <h2>Password Assistance</h2>
          <p>To authenticate, please click on the Reset Password link below. It will expire in 10 minutes.</p>
          ${link}
          ...
          <p>We hope to see you again soon.</p>	
			...
			let info = await transporter.sendMail({
				...
				to: emailToUser, // list of receivers
